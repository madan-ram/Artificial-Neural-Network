...
def runner(path):
	...
	img = cv2.imread(path)
	try:
		img = cv2.resize(img, (50, 65)) / 255.0
		maps = []
		maps.append(img[:,:,0])
		maps.append(img[:,:,1])
		maps.append(img[:,:,2])
...
manager = Manager()
net = buildNetwork(147*16*12, int(math.sqrt(147*16*12)), int(math.sqrt(int(math.sqrt(147*16*12)))), 5, bias=True, hiddenclass=SigmoidLayer, outclass=SoftmaxLayer)
...
maps_list = manager.list([])
pool = multi.Pool(processes=4)
pool.map(runner, fileNames_path)
pool.close()
for maps, path in maps_list:
